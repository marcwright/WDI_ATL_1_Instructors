...
describe FortuneCookie do
  ...
  describe "#read" do
    it "tells you to unwrap the cookie if it's still wrapped" do
      expect( fortune.read ).to eq("Unwrap me!")
    end

    # context and before are friends!
    context "When the fortune cookie is unwrapped" do
      before { fortune.unwrap! }

      it "tells you to crack the cookie if it's still uncracked" do
        expect( fortune.read ).to eq("Crack me!")
      end

      it "reads the fortune if the cookie is unwrapped and cracked" do
        fortune.crack!
