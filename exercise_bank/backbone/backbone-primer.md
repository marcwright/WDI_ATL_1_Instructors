...
There are numerous [popular template libraries](http://garann.github.io/template-chooser/) available, although these additional toolsets are technically unnecessary given that Underscore has a [perfectly capable template renderer](http://underscorejs.org/#template) built in, and is thus omnipresent all Backbone apps. For this primer, we'll be using the Underscore template renderer.
